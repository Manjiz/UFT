var applyState = {
        ...
        controller: function($rootScope, $scope, $http, $timeout, $stateParams) {
            ...
            $scope.customOptions = {
                dataSource: {
                    transport: {
                        read: {
                            dataType: "json",
                            url: 'http://' + location.host + '/api/business/listForUser',
                        }
                    }
                },
                dataTextField: "name",
            ...
            $scope.datePickerOpts = {
                value: $stateParams.date&&Math.floor(new Date($stateParams.date)/86400000)>=Math.floor(new Date()/86400000) ? $stateParams.date : new Date(),
                format: "yyyy-MM-dd",
                min: new Date(),
            ...
            $scope.userObjSelectedOpts = {
                ...
                value: $stateParams.erp ? '('+$stateParams.erp+')' : ''
            }
        }
    }
