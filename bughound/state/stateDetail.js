var detailState = {
        ...
        controller: function($rootScope, $scope, $http, $sce, $stateParams, $timeout, SessionService, datalump) {
            /*-------------------------------------------------------
                ROOT: uerp
                STATE: 
            ...
            $scope.$on('authdone', function() {
                $http.get('api/demand/get?demandID='+$stateParams.did).success(function (data) {
                    ...
                    // 身份信息
                    $scope.identity = {
                        isAnonymus: SessionService.isAnonymus,
                        isAdmin: SessionService.session.isAdmin,
                        isSubmitter: demand.submitter==SessionService.session.loginederp,
                        isExecutor: demand.executor==SessionService.session.loginederp,
                        isAssignee: demand.assignee==SessionService.session.loginederp
                    };
                    // 需求信息
                    $scope.demand = {
