var scheduleState = {
        ...
        controller: function($rootScope, $scope, $http, $state, SessionService, datalump) {
            var dataList,   //排期数据
                thisWeekStart;  //周起始日期
            ...
            $scope.$on('authdone', function() {
                $http.get('api/user/getbyerp', {
                    params: {
                        erp: SessionService.session.loginederp
                    }
            ...
            $scope.changeSchedule = function(sid, did, dname, pm, dictator, assignee, startDate, days) {
                if(SessionService.session && SessionService.session.isAdmin) {
