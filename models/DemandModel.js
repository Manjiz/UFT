	...
	add: function(req, res) {
		pool.getConnection(function(err, conn) {
			var submitter = req.session.loginederp,	//申请人
				submitDate = new Date(),			//申请时间
				demandID = new Date().toISOString().replace(/(\s)|T|(\..+)|:|-/g, ''),	//---20150918064419
			...
			// 文件从temp转移到upload
			fs.exists('upload', function(isExists) {
				if(!isExists) {
					fs.mkdir('upload');
				}
				files.forEach(function(e, i, arr) {
					var src = fs.createReadStream('temp/' + e.filename),
						dest = fs.createWriteStream('upload/' + e.filename);
					src.pipe(dest);
  					src.on('end', function() { });
  					src.on('error', function(err) {  });
  					filesForMySql.push({filename:e.filename, filepath:'upload/'+e.filename, originalname: e.originalname, mimetype: e.mimetype, uerp:req.session.loginederp});
				})
			})
			if(dtype && business && name) {
	...
	getMyDemands: function(req, res) {
		pool.getConnection(function(err, conn) {
			...
			conn.query('SELECT '
				...
				+ 'WHERE (demand.submitter=? OR demand.executor=?) AND state=?', [uerp, uerp, state], function(err, rows, fields) {
					if(err) throw err;
					if(rows.length>0) {
	...
	getNotAuditedDemands: function(req, res) {
		pool.getConnection(function(err, conn) {
			conn.query('SELECT '
				...
				+ 'LEFT JOIN user as user_executor ON demand.executor=user_executor.erp '
				+ 'LEFT JOIN user as user_solver ON demand.solver=user_solver.erp '
				+ 'WHERE state=0 AND demand.status=0', function(err, rows, fields) {
					if(err) throw err;
					if(rows.length>0) {
						res.json({state:'success', msg:'查找需求成功', data:JSON.stringify(rows)});
					} else {
	...
	changeState: function(req, res) {
		...
		if(filesForMySql.length>0 && demandid && type) {
			pool.getConnection(function(err, conn) {
				...
				// conn.query('SELECT files FROM demand WHERE demandID=?', [demandid], function(err, rows, fields) {
				// 	if(err) throw err;
				// 	if(rows.length>0) {
				// 		filesForMySql = JSON.parse(rows[0].files).concat(filesForMySql);
				// 		conn.query('UPDATE demand SET files=? WHERE demandID=?', [JSON.stringify(filesForMySql), demandid], function(err, result) {
				// 			if(err) throw err;
				// 			if(result.affectedRows>0) {
				// 				console.log('添加附件成功')
				// 			}
		...
		if(demandid && type) {
			console.log(startDate, endDate, new Date(startDate), new Date(endDate))
			pool.getConnection(function(err, conn) {
				if(type=='audited') {
					// 验证管理员身份
					conn.query('SELECT * FROM user WHERE erp=? AND isAdmin=1 AND status=0', [handler], function(err, rows, fields) {
						if(err) throw err;
						console.log('>>> demand/audited::身份验证通过');
						if(rows.length>0) {
							conn.query('UPDATE demand SET state=1, solveMsg=CONCAT_WS("", solveMsg, ?), solver=?, executor=? WHERE demandID=? AND status=0', [solveMsg, handler, designated, demandid], function(err, result) {
								if(err) throw err;
								if(result.affectedRows>0) {
									conn.query('INSERT INTO schedule (demandID, dictator, assignee, startDate, endDate) VALUES (?,?,?,?,?)', [demandid, handler, designated, new Date(startDate), new Date(endDate)], function(err, result) {
										if(err) throw err;
										if(result.affectedRows>0) {
											demandStateEmail(demandid, handler+'审核通过了需求，请处理人受理', 'http://'+req.headers.host+'/#/detail/'+demandid);
											console.log('>>> demand/audited::需求审核通过');
					...
					conn.query('SELECT * FROM user WHERE erp=? AND isAdmin=1 AND status=0', [handler], function(err, rows, fields) {
						...
						if(rows.length>0) {
							...
							conn.query('UPDATE demand SET state=4, solveMsg=CONCAT_WS("", solveMsg, ?), solver=? WHERE demandID=?', [solveMsg, handler, demandid], function(err, result) {
								if(err) throw err;
								if(result.affectedRows>0) {
									demandStateEmail(demandid, '需求被'+handler+'关闭，周知', 'http://'+req.headers.host+'/#/detail/'+demandid);
					...
					conn.query('SELECT * FROM user WHERE erp=? AND isAdmin=1 AND status=0', [handler], function(err, rows, fields) {
						...
						if(rows.length>0) {
							...
							conn.query('UPDATE demand SET state=1, solveMsg=CONCAT_WS("", solveMsg, ?), solver=? WHERE demandID=?', [solveMsg, handler, demandid], function(err, result) {
								if(err) throw err;
								if(result.affectedRows>0) {
									demandStateEmail(demandid, '需求被'+handler+'重启，周知', 'http://'+req.headers.host+'/#/detail/'+demandid);
						...
						if(rows.length>0) {
							if(!rows[0].executor || rows[0].executor==handler) {	// 如果不限制执行人 或 当前就是执行人
								...
								conn.query('UPDATE demand SET state=2, solveMsg=CONCAT_WS("", solveMsg, ?), solver=? WHERE demandID=? AND state<>0 AND state<>4', [solveMsg, handler, demandid], function(err, result) {
									if(err) throw err;
									if(result.affectedRows>0) {
										demandStateEmail(demandid, '需求已被'+handler+'受理', 'http://'+req.headers.host+'/#/detail/'+demandid);
						...
						if(rows.length>0) {
							if(!rows[0].executor || rows[0].executor==handler) {	// 如果不限制执行人 或 当前就是执行人
								...
								conn.query('UPDATE demand SET state=3, solveMsg=CONCAT_WS("", solveMsg, ?), solver=?, solveDate=? WHERE demandID=? AND state<>0 AND state<>4', [solveMsg, handler, new Date, demandid], function(err, result) {
									if(err) throw err;
									if(result.affectedRows>0) {
										demandStateEmail(demandid, handler+'已完成需求', 'http://'+req.headers.host+'/#/detail/'+demandid);
							...
							conn.query('SELECT * FROM user WHERE erp=? AND status=0', [designated], function(err, rows, fields) {
								...
								if(rows.length>0 || !designated) {	//未指派或找到该erp
									...
									conn.query('UPDATE demand SET solveMsg=CONCAT_WS("", solveMsg, ?), solver=?, executor=? WHERE demandID=?', [solveMsg, handler, designated, demandid], function(err, result) {
										if(err) throw err;
										if(result.affectedRows>0) {
											demandStateEmail(demandid, handler+'指派'+designated, 'http://'+req.headers.host+'/#/detail/'+demandid);
	...
	getTopSubmitter: function(req, res) {
		pool.getConnection(function(err, conn) {
			conn.query('SELECT '
				...
				if(err) throw err;
				res.json({state:'success', data:rows});
				conn.release();
			})
		});
	}
...
var emailTpl = '<div style="text-align: center;color: #333;line-height: 40px;font-size: 12px;">此邮件乃自动发送，请勿直接回复</div>'
				+'<div style="width: 640px;margin: 0 auto;background-color: #fff;">'
					+'<div style="text-align: center;color: #000;font-size: 22px;padding: 20px 0;border-bottom: 1px dashed #DDDDDD;margin: 0 20px;">'
						+'<img src="http://labs.qiang.it/tools/pp/upload/20150917-832.png" alt="" style="display: block;margin: 0 auto;">'
					+'</div>'
