	...
	this.each(function(){
	  ...
	  var fileObj = {
		  ...
		  onSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){console.log(files)
					...
					if(option.showUploadedPercent){
						var percentText = '<div class="up_percent">0%</div>';
						$html.find('.uploadify-progress').html(percentText);
					}
					//判断是否是自动上传
					if(option.auto){
		  ...
		  onProgress: function(file, loaded, total) {
				...
				if(option.showUploadedPercent){
					eleProgress.children('.up_percent').text(percent);	
				}
		  ...
		  funDeleteFile: function(index) {
			  for (var i = 0,len=this.fileFilter.length; i<len; i++) {
					  ...
					  if (file.index == index) {
						  ...
						  console.log('#fileupload_'+instanceNumber+'_'+index)
				  ...
				  xhr.onreadystatechange = function(e) {
					  if (xhr.readyState == 4) {
						  if (xhr.status == 200) {
							  ...
							  // 填充数据啊
							  var json = eval('('+xhr.responseText+')');console.log(json)
							  // console.log(json, '#fileupload_'+instanceNumber+'_'+file.index)
							  _this.find('#fileupload_'+instanceNumber+'_'+file.index).attr('data-originalname', json.originalname).attr('data-filename', json.filename).attr('data-mimetype', json.mimetype);

							  //在指定的间隔时间后删掉进度条
							  setTimeout(function(){
							  	_this.find('#fileupload_'+instanceNumber+'_'+file.index+' .uploadify-progress').hide();
